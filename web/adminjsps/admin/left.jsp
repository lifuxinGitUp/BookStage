...
<html>
  <head>
	...
	-->
	  <style type="text/css">
		  body{
			  background: url("/storephoto/adminbody.jpg");
		  }
	  </style>
...
function load() {
	...
	bar1.add("订单管理", "未付款订单", "<c:url value='/AdminOrderServlet?method=noPay&state=1'/>", "body");
	bar1.add("订单管理", "已付款订单", "<c:url value='/AdminOrderServlet?method=noPay&state=2'/>", "body");
	bar1.add("订单管理", "未收货订单", "<c:url value='/AdminOrderServlet?method=noPay&state=3'/>", "body");
	bar1.add("订单管理", "已完成订单", "<c:url value='/AdminOrderServlet?method=noPay&state=4'/>", "body");
